...
def draw_pose(pose, H, W, draw_aug=False):
    bodies = pose["bodies"]
    faces = pose["faces"]
    hands = pose["hands"]
    ...
    if not draw_aug:
        ...
        canvas = util.draw_bodypose_aug(canvas, candidate, subset)
        canvas = util.draw_handpose_aug(canvas, hands)
        canvas = util.draw_facepose_aug(canvas, faces)

    return canvas


class DWposeDetector:
    def __init__(self, draw_aug=False):
        self.draw_aug = draw_aug
        pass
    ...
    def __call__(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            detected_map = draw_pose(pose, H, W, self.draw_aug)
